...
int map_load(Elf64_Phdr* s_ph, mdata_binary_t* s_binary) 
{
    ...
    if (!s_binary->base) {
        ...
        if (MAP_FAILED == mmap((void *)PAGE_ALIGN(curr_map), PAGE_ROUND((PAGE_ROUND((curr_map + s_ph->p_filesz)) - PAGE_ALIGN(curr_map))) + 1, PROT_READ | PROT_WRITE | _PROT_EXEC(s_ph->p_flags), MAP_FIXED | MAP_PRIVATE, s_binary->fd, PAGE_ALIGN(s_ph->p_offset))) {
            return -1;
        }
    }
